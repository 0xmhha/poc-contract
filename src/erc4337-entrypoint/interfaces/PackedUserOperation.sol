 ...
 * and paymaster-specific extra data
 *                                The paymaster will pay for the transaction instead of the sender.
 * @param signature - Sender-verified signature over the entire request, the EntryPoint address and the chain ID.
 *
 *
 * Field layout (enforced on-chain by EntryPoint):
 * - sender: must already be deployed, or be the address that `initCode` will deploy; for EIP-7702 onboarding, `initCode
 * = 0x7702 || optionalPayload`
 *   and `sender.code` must begin `0xef0100 || delegate`.
 * - nonce = uint192(key) || uint64(sequence); EntryPoint tracks sequential values of `sequence` separately for each
 * `key` value.
 * - initCode:
 ...
 *     * 7702: `0x7702` (magic prefix), optionally padded to 20 bytes and followed by the actual `initializationCode`
 * data. This optional payload is executed on `sender` to finalise delegate setup.
 * - callData: executed verbatim; if it starts with `IAccountExecute.executeUserOp.selector` (0x8dd7712f), EntryPoint
 * wraps and forwards `(userOp, userOpHash)`.
 ...
 * - paymasterAndData (if non-empty) = `paymaster(20) || verificationGasLimit(16) || postOpGasLimit(16) ||
 * paymasterData`
