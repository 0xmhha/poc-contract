...
contract PolicyHook is IHook {
        ...
        returns (bytes memory hookData)
    {
        ...
        // Extract target from msgData (first 20 bytes)
        if (msgData.length < 20) {
            return abi.encode(address(0), uint256(0));
        }

        address target = address(bytes20(msgData[0:20]));

        // Check policy mode
        if (store.mode == PolicyMode.ALLOWLIST) {
            _checkAllowlistPolicy(store, target, msgValue, msgData);
        } else {
            _checkBlocklistPolicy(store, target, msgValue, msgData);
        }

        // Check and record transaction count
        _checkAndRecordTxCount(store);

        return abi.encode(target, msgValue);
    }
    ...
    function _checkAllowlistPolicy(
        ...
        if (config.strictSelectorMode && msgData.length >= 56) {
            // 20 (target) + 32 (value) + 4 (selector) = 56
