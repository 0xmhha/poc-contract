 ...
 *
 * Deploys Fallback modules using forge script
 ...
 *
 * Usage:
 *   npx ts-node script/ts/deploy-fallbacks.ts [--broadcast] [--verify] [--force]
 *
 * Options:
 ...
 *
 * Examples:
 *   npx ts-node script/ts/deploy-fallbacks.ts                    # Dry run
 *   npx ts-node script/ts/deploy-fallbacks.ts --broadcast        # Deploy
 *   npx ts-node script/ts/deploy-fallbacks.ts --verify           # Verify only
 *   npx ts-node script/ts/deploy-fallbacks.ts --broadcast --verify  # Deploy + verify
 */
...
const FORGE_SCRIPT = "script/deploy-contract/DeployFallbacks.s.sol:DeployFallbacksScript";
const FOUNDRY_PROFILE = "fallbacks";

// Contract names, artifacts, and their JSON keys
const CONTRACTS = [
  { name: "TokenReceiverFallback", artifact: "src/erc7579-fallbacks/TokenReceiverFallback.sol:TokenReceiverFallback", jsonKey: "tokenReceiverFallback" },
  { name: "FlashLoanFallback", artifact: "src/erc7579-fallbacks/FlashLoanFallback.sol:FlashLoanFallback", jsonKey: "flashLoanFallback" },
];
...
function verifyContracts(chainId: string): void {
  ...
  if (!hasAnyAddress) {
    console.log("No deployed Fallback addresses found to verify");
    return;
  }
  ...
  // All fallbacks have no constructor arguments
...
function main(): void {
  ...
  console.log("=".repeat(60));
  console.log("  ERC-7579 Fallbacks Deployment");
  console.log("=".repeat(60));
  ...
  try {
    ...
    console.log("\n" + "=".repeat(60));
    if (broadcast) {
      console.log("✅ Fallbacks deployment completed!");
      ...
      console.log("\nNext steps:");
      console.log("  1. Deploy executors: ./script/deploy-executors.sh --broadcast");
      console.log("  2. Install fallbacks on Smart Accounts via installModule()");
      console.log("\nFallback Use Cases:");
      console.log("  - TokenReceiverFallback: NFT marketplace, airdrops, DeFi LP tokens");
      console.log("  - FlashLoanFallback: Arbitrage, liquidations, collateral swaps");
    } else {
