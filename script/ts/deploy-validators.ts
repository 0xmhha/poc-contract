 ...
 *
 * Deploys Validator modules using forge script
 ...
 *
 * Usage:
 *   npx ts-node script/ts/deploy-validators.ts [--broadcast] [--verify] [--force]
 *
 * Options:
 ...
 *
 * Examples:
 *   npx ts-node script/ts/deploy-validators.ts                    # Dry run
 *   npx ts-node script/ts/deploy-validators.ts --broadcast        # Deploy
 *   npx ts-node script/ts/deploy-validators.ts --verify           # Verify only
 *   npx ts-node script/ts/deploy-validators.ts --broadcast --verify  # Deploy + verify
 */
...
function verifyContracts(chainId: string): void {
  ...
  if (!hasAnyAddress) {
    console.log("No deployed Validator addresses found to verify");
    return;
  }
  ...
  // All validators have no constructor arguments
...
function main(): void {
  ...
  console.log("=".repeat(60));
  console.log("  ERC-7579 Validators Deployment");
  console.log("=".repeat(60));
  ...
  try {
    ...
    console.log("\n" + "=".repeat(60));
    if (broadcast) {
      console.log("✅ Validators deployment completed!");
      ...
      console.log("\nNext steps:");
      console.log("  1. Deploy hooks: ./script/deploy-hooks.sh --broadcast");
      console.log("  2. Install validators on Smart Accounts via installModule()");
    } else {
